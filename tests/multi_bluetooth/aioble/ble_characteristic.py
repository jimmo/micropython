...
async def instance0_task():
    ...
    multitest.globals(BDADDR=aioble.ble.config("mac"))
    multitest.next()
    ...
    print("written", characteristic.read())
    print("notify")
    characteristic.notify(device, "periph2")
    ...
    print("indicate", await characteristic.indicate(device, timeout_ms=TIMEOUT_MS))

    # Wait for the central to disconnect.
    await device.disconnected(timeout_ms=TIMEOUT_MS)
    print("disconnected")
...
async def instance1_task():
    multitest.next()
    x = asyncio.PollingEvent()

    # Connect to peripheral and then disconnect.
    print("connect")
    device = aioble.Device(*BDADDR)
    await device.connect(timeout_ms=TIMEOUT_MS)
    ...
    print("disconnect")
    await device.disconnect(timeout_ms=TIMEOUT_MS)
    print("disconnected")
