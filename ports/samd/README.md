...
| ------------------------------------------------------------ | ----------------------------------------------------------- | --------------- | ------ |
| ADAFRUIT_FEATHER_M0_EXPRESS                                  | Tx=PB22=SERCOM5/PAD[2], Rx=PB23=SERCOM5/PAD[3]              | 64k             | No     |
...
| WIO_TERMINAL_D51R                                            | Tx=BCM14=PB27=SERCOM2/PAD[0], Rx=BCM15=PB26=SERCOM2/PAD[1]  | 128k            | Yes    |
| XIAO_M0                                                      | Tx=A6=PB8=SERCOM4/PAD[0], Rx=A7=PB9=SERCOM4/PAD[1]          | 64k             | Yes    |
...
- Each board has its own pin numbering scheme, so please see the table below (the
  ...
  (index) by 'Pin'. Eg; `XIAO_M0/pins.c`: `{{&machine_pin_type}, PIN_PA02}, // A0/D0`
  means MicroPython `Pin(0)` is XIAO_M0 pin "A0/D0" on SAMD21G18A PortA, Pin2.

- Note: on the XIAO_M0, if the TX & TX pins are used by the `Pin()` class, the `Pin()`
...
Eg; `XIAO_M0/pins.c`: `{{&machine_led_type}, PIN_PA17}, // W13` means MicroPython
`LED(0)` is XIAO_M0 LED "W13" connected to SAMD21G18A PortA, Pin17.
