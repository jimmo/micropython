...
def _show_progress_bar(size, total_size):
    ...
    if command == "cp" and args.recursive:
        ...
        for dir, file in src_files:
            ...
            pyboard.filesystem_command(
                ...
                progress_callback=_show_progress_bar,
                verbose=verbose,
            )
    else:
        if args.recursive:
        ...
        try:
            pyboard.filesystem_command(
                state.pyb, [command] + paths, progress_callback=_show_progress_bar, verbose=verbose
            )
...
def do_edit(state, args):
    ...
    for src in args.files:
        ...
        try:
            ...
            state.pyb.fs_get(src, dest, progress_callback=_show_progress_bar)
            if os.system("$EDITOR '%s'" % (dest,)) == 0:
                state.pyb.fs_put(dest, src, progress_callback=_show_progress_bar)
        finally:
            os.unlink(dest)
