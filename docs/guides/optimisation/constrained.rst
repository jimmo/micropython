.. _guides_optimisation_constrained:
...
to control GC. These are to be found in the :mod:`gc` and :mod:`micropython`
modules. The following example may be pasted at the REPL (``ctrl e`` to enter
paste mode, ``ctrl d`` to run it).
