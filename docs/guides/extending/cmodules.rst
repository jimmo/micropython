...
If your limitations can't be resolved with suggestions in :ref:`guides_optimisation_speed_python`,
...
An alternative approach is to use :ref:`guides_extending_natmod` which allows
writing custom C code that is placed in a .mpy file, which can be imported
dynamically in to a running MicroPython system without the need to recompile
the main firmware.
