...
One of the main advantages of using native .mpy files is that native machine
code can be imported by a script dynamically, without the need to rebuild the
main MicroPython firmware.  This is in contrast to :ref:`guides_extending_cmodules`
which also allows defining custom modules in C but they must be compiled into
...
details about .mpy files see :ref:`guides_implementation_mpyfiles`.
