...
MicroPython can be compiled as a regular program for UNIX (Linux, macOS) or
Windows, and operates in much the same way as CPython's ``python`` binary.

This can be useful for both developing MicroPython itself and for building or
prototyping MicroPython-based applications without needing to flash code to a
microcontroller.
