...
.. glossary::
    ...
    bare-metal
        ...
        :term:`MCU`-based system. When running on a bare-metal system,
    ...
    compiler
        The code running in the MicroPython firmware that turns your Python code
        into :term:`bytecode` (or native code, see :term:`emitter`).
    ...
    cross-compiler
        ...
        See the :ref:`cross-compiler guide <guides_workflow_crosscompiling>`.
    ...
    emitter
        The :term:`compiler` uses an emitter to generate either :term:`bytecode`
        (via the "bytecode emitter") or :term:`native` code (via the "native emitter")
        specific to the architecture of the device.

    FFI
        Acronym for Foreign Function Interface. A mechanism used by the
        :term:`Unix port` to access operating system functionality.
        This is not available on :term:`bare-metal` ports.
    ...
    filesystem
        ...
        See the :ref:`filesystem guide <guides_software_filesystem>`.
    ...
    frozen module
        ...
        See the :ref:`frozen code guide <guides_workflow_freezing>`.

    garbage collector
        A background process that runs in Python (and MicroPython) to reclaim
        unused memory in the :term:`heap`.

        See the :mod:`gc` module and the :ref:`gc optimisation guide <guides_optimisation_gc>`.
    ...
    interned string
        ...
        For more information see the :ref:`QSTR implementation details <guides_implementation_qstr>`.

    MCU
        Short for "microcontroller". Microcontrollers usually have much less
        resources than a desktop, laptop, or phone processor, but are smaller,
        cheaper and require much less power. MicroPython is designed to be small and
    ...
    micropython-lib
        ...
        only be used on the :term:`Unix port` (with limited support
        for Windows).
    ...
    .mpy file
        ...
        For more information see the :ref:`.mpy implementation details <guides_implementation_mpyfiles>`.

    native
        Usually refers to "native code", i.e. machine code for the target
        microcontroller (such as ARM Thumb, Xtensa, x86/x64). The ``@micropython.native``
        (or ``@micropython.viper``) decorator can be applied to a MicroPython function to
        generate native code instead of :term:`bytecode` for that function, which will
        likely be faster but use more RAM.

        For more information see the :ref:`guide to using the native emitter <guides_optimisation_native>`.

    port
        MicroPython supports different :term:`boards <board>`,
        :term:`RTOSes <RTOS>`, and OSes, and can be relatively easily adapted to
        new systems. MicroPython with support for a particular system is
        called a "port" to that system. Different ports may have widely
        different functionality. This documentation is intended to be a
        reference of the generic APIs available across different ports
        ("MicroPython core"). Note that some ports may still omit some APIs
        described here (e.g. due to resource constraints). Any such
        differences, and port-specific extensions beyond the MicroPython core
        functionality, would be described in the separate port-specific
        documentation.

        For more information see the :ref:`supported devices section <devices>`.

    .py file
        A file containing Python source code.

    QSTR
        See :term:`interned string`.
    ...
    REPL
        ...
        For more information see the :ref:`REPL guide <guides_workflow_repl>`.
    ...
    stream
        ...
        sequential read-write access to the underlying data, such as a file or socket.

        A stream object implements a corresponding interface, which consists of
        methods like ``read()``, ``write()``, ``readinto()``, ``seek()``, ``flush()``,
        ``close()``, etc. A stream is an important concept in MicroPython and many
        I/O objects implement the stream interface, and thus can be used consistently
        and interchangeably in different contexts.

        For more information on streams in MicroPython, see the :mod:`io` module.
    ...
    Unix port
        The unix port is one of the major :term:`MicroPython ports <port>`. It
        is intended to run on POSIX-compatible operating systems, like Linux,
        MacOS, FreeBSD, Solaris, etc. It also serves as the basis of Windows
        port. The Unix port is very useful for quick development and testing of
        the MicroPython language and machine-independent features. It can also
        function in a similar way to :term:`CPython`'s ``python`` executable.
