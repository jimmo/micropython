...
.. function:: ssl.wrap_socket(sock, server_side=False, keyfile=None, certfile=None, cert_reqs=CERT_NONE, cadata=None, server_hostname=None, do_handshake=True)
    ...
    Takes a `stream` *sock* (usually a :class:`socket.socket` instance
    of :data:`socket.SOCK_STREAM` type), and returns a "SSL-wrapped" `stream`.

    The returned object is a `stream`, not a :class:`socket.socket` (see below).
   ...
   - *server_hostname* is for use as a client, and sets the hostname to check against the received
    ...
    :term:`port`, some or all keyword arguments above may be not supported.

.. warning::

    Some implementations of the :mod:`ssl` module do NOT validate server certificates,
    which makes an SSL connection established prone to man-in-the-middle attacks.

    CPython's :meth:`python:socket.socket.wrap_socket` returns
    an :class:`python:ssl.SSLSocket` object which has methods typical for
    sockets, such as :meth:`python:socket.socket.send`,
    :meth:`python:socket.socket.recv`, etc.

    MicroPython's :meth:`socket.socket.wrap_socket` returns an object more
    similar to CPython's :class:`python:ssl.SSLObject` which does not have
    these socket methods. Instead use the `stream` methods, ``read()``,
    ``write()``, etc.
...
.. data:: ssl.SSLError
    ...
    This exception does NOT exist in MicroPython.

    Instead its base class, :exc:`OSError`, is used.
