...
The :mod:`machine` module contains specific functions related to the hardware on a
particular board. Most functions in this module allow direct and unrestricted
access to and control of hardware blocks on a system(like CPU, timers, buses,
etc.). Used incorrectly, this can lead to malfunction, lockups, crashes of your
board, and in extreme cases, hardware damage.

.. _machine_callbacks:

Various functions and class methods of this module use callbacks to
asynchronously signal events from peripherals. These callbacks should be
considered as executing in an interrupt context. This is true for both physical
devices with IDs >= 0 and "virtual" devices with negative IDs like -1
(these "virtual" devices are still thin shims on top of real hardware and real
hardware interrupts). See :ref:`guides_software_interrupts`. TODO: this could
be more nuanced.
...
The following classes are provided for access to specific peripherals:
...
* :class:`SPI` - Serial Peripheral Interface bus protocol
* :class:`Timer` - control hardware timers
* :class:`TouchPad` - capacitive touchpad
...
For more information, see the `boot and reset guide <guides_software_boot_reset>`.
...
.. function:: soft_reset()
    ...
    It tries to retain the method by which the user is connected to the
    MicroPython REPL (e.g. serial, USB, WiFi).
...
.. function:: lightsleep([time_ms])
    ...
    * A deepsleep may not retain RAM or any other state of the system
      (for example peripherals or network interfaces).  Upon wake execution is
      resumed from the main script, similar to a hard or power-on reset. The
      `reset_cause()` function will return :data:`machine.DEEPSLEEP` and this
      can be used to distinguish a deepsleep wake from other resets.
...
.. function:: time_pulse_us(pin, pulse_level, timeout_us=1000000, /)
    ...
    the function f (*) waits until the pin input becomes equal to *pulse_level*,
