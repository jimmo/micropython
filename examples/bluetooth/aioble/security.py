...
from .core import log_info, log_warn, ble
...
def _security_irq(event, data):
    ...
    if event == _IRQ_ENCRYPTION_UPDATE:
        ...
        if device := Device._connected.get(conn_handle, None):
            ...
            # TODO: Handle failure.
            if encrypted and device._pair_event:
                device._pair_event.set()
...
async def pair_device(
    ...
    with device.timeout(timeout_ms):
        device._pair_event = asyncio.PollingEvent()
        ble.gap_pair(device._conn_handle)
        await device._pair_event.wait()
