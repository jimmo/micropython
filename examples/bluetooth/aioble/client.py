...
from .core import ble, GattError
...
class ClientDiscover:
    def __init__(self, device, disc_type, parent, *args):
        ...
        # This will be the device for a service discovery, and the service for a characteristic discovery.
        self._parent = parent
    ...
    async def _start(self):
        if self._device._discover:
            # TODO: cancel existing? (e.g. perhaps they didn't let the loop run to completion)
            raise ValueError("Discovery in progress")

        # Tell the device that we're the active discovery operation (the IRQ only gives us conn_handle).
    ...
    # Tell the active discovery instance for this device to add a new result
    # to the queue.
    def _discover_result(conn_handle, *args):
        if device := Device._connected.get(conn_handle, None):
            if discover := device._discover:
                discover._queue.append(args)
                discover._event.set()

    # Tell the active discovery instance for this device that it is complete.
...
# Represents a single service supported by a device. Do not construct this
# class directly, instead use `async for service in device.services([uuid])` or
# `await device.service(uuid)`.
class ClientService:
    ...
    def characteristics(self, uuid=None):
        return ClientDiscover(self.device, ClientCharacteristic, self, uuid)

    # For ClientDiscover
    def _start_discovery(device, uuid=None):
...
class BaseClientCharacteristic:
    ...
    def _register_with_device(self):
        self._device()._characteristics[self._value_handle] = self

    # Map an incoming IRQ to an registered characteristic.
    def _find(conn_handle, value_handle):
        if device := Device._connected.get(conn_handle, None):
            if characteristic := device._characteristics.get(value_handle, None):
    ...
    async def read(self, timeout_ms=1000):
        ...
        self._register_with_device()
        # This will be set by the done IRQ.
        self._read_status = None
        # This will be set by the result and done IRQs. Re-use if possible.
        self._read_event = self._read_event or asyncio.PollingEvent()

        # Issue the read.
        ble.gattc_read(self._device()._conn_handle, self._value_handle)

        with self._device().timeout(timeout_ms):
    ...
    async def write(self, data, response=False, timeout_ms=1000):
        if response:
            ...
            self._write_event = self._write_event or asyncio.PollingEvent()

        # Issue the write.
        ble.gattc_write(self._device()._conn_handle, self._value_handle, data, response)
...
class ClientCharacteristic(BaseClientCharacteristic):
    ...
    async def descriptor(self, uuid):
        result = None
        # Make sure loop runs to completion.
        async for descriptor in self.descriptors():
    ...
    def descriptors(self):
        return ClientDiscover(self.device, ClientDescriptor, self)

    # For ClientDiscover
    def _start_discovery(service, uuid=None):
        ble.gattc_discover_characteristics(
            service.device._conn_handle, service._start_handle, service._end_handle, uuid
        )
    ...
    async def notified(self, timeout_ms=None):
        ...
        if data is None:
            self._notify_event = self._notify_event or asyncio.PollingEvent()
            with self._device().timeout(timeout_ms):
    ...
    async def indicated(self, timeout_ms=None):
        ...
        if data is None:
            self._indicate_event = self._indicate_event or asyncio.PollingEvent()
            with self._device().timeout(timeout_ms):
...
class ClientDescriptor(BaseClientCharacteristic):
    ...
    def _device(self):
        return self.characteristic.service.device

    # For ClientDiscover
    def _start_discovery(characteristic, uuid=None):
        ble.gattc_discover_descriptors(
            characteristic._device()._conn_handle,
