...
def _peripheral_irq(event, data):
    ...
    if event == _IRQ_CENTRAL_CONNECT:
        ...
        _incoming_connection = Device(addr_type, bytes(addr))
        _incoming_connection._conn_handle = conn_handle
        Device._connected[conn_handle] = _incoming_connection
...
async def advertise(
    ...
    _connect_event = _connect_event or asyncio.PollingEvent()
    ...
    try:
        ...
        # Get the newly connected (central) device and start a task to wait
        # for disconnection.
        result = _incoming_connection
        _incoming_connection = None
        # This mirrors what connecting to a central does.
        result._run_device_task()
        return result
