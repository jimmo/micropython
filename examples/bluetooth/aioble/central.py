    ...
    # it in the pending connection set.
        ...
        for d in _connecting:
            if d.addr_type == addr_type and d.addr == addr:
                ...
                d._conn_handle = conn_handle
                d._event.set()
                break

    # Find the active device for this connection handle.
...
async def _connect(device, timeout_ms):
    if device._conn_handle is not None or device in _connecting:
        return
    ...
    device._event = device._event or asyncio.PollingEvent()
    ...
    try:
        with DeviceTimeout(None, timeout_ms):
            ...
            await device._event.wait()
            assert device._conn_handle is not None

            # Register connection handle -> device.
            Device._connected[device._conn_handle] = device
    finally:
